...
output_path = str(pathlib.Path(args.outputFolder) / (pathlib.Path(args.bam).stem + '_traceome_v4.pkl'))


if not os.path.exists(output_path) or overwrite:
    cmd = f'{pythonCmd} scripts/Traceome_v4.py -i {sorted_bam} -b {cbr_bed} -o {outputFolder}'
    run_command(cmd, 'Transcriptome')
